...
# ---------------------------------------------------------------------------
# Reuse existing bundled-mode flag (same as langchain_gpt_api.py:329)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Circuit breaker (same as hevolve_backend_adapter.py:49-52)
# ---------------------------------------------------------------------------
_tier1_fail_count = 0
_tier2_fail_count = 0
_FAIL_THRESHOLD = 2
...
def execute_vlm_instruction(message: dict) -> dict | None:
    ...
    # Tier 1: In-process (bundled + deps available + circuit breaker open)
    if _BUNDLED_MODE and _HAS_PYAUTOGUI and _tier1_fail_count < _FAIL_THRESHOLD:
        try:
...
def check_vlm_available() -> bool:
    ...
    """
    # Tier 1: always available if bundled + pyautogui present
