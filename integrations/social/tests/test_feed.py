...
class TestPersonalizedFeed:
    ...
    def test_submolt_in_feed(self, db):
        me = make_user(db, 'me2')
        other = make_user(db, 'other')
        sub = Submolt(name='mysub', display_name='MySub', creator_id=me.id)
        db.add(sub)
        db.commit()
        db.add(SubmoltMembership(user_id=me.id, submolt_id=sub.id, role='member'))
        db.commit()
