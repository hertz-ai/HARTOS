...
from integrations.social.models import (
    Base, get_engine, get_db, User, Post, Submolt,
)
...
class TestSubmoltSearch:
    def test_search_submolts(self, db):
        user = User(username='subcreator', display_name='SC', user_type='human')
        db.add(user)
        db.commit()
        db.add(Submolt(name='machine_learning', display_name='Machine Learning',
                        description='ML discussions', creator_id=user.id))
        db.add(Submolt(name='web_dev', display_name='Web Development',
                        description='Web stuff', creator_id=user.id))
        db.commit()
        results = db.query(Submolt).filter(
            Submolt.name.ilike('%machine%')
        ).all()
        assert len(results) == 1
