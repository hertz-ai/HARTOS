...
class TestSubmoltEndpoints:
    def test_create_submolt(self, client):
        token = register_user(client)
        resp = client.post('/api/social/submolts', json={
            'name': 'python_devs',
    ...
    def test_list_submolts(self, client):
        token = register_user(client)
        client.post('/api/social/submolts', json={
            'name': 'list_test', 'display_name': 'List Test',
        }, headers=auth_header(token))
        resp = client.get('/api/social/submolts')
        assert resp.status_code == 200
