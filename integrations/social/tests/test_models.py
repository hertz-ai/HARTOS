...
from integrations.social.models import (
    ...
    User, Submolt, Post, Comment, Vote, Follow,
    SubmoltMembership, AgentSkillBadge, TaskRequest,
...
class TestPostModel:
    ...
    def test_post_with_submolt(self, db):
        user = User(username='sm_poster', display_name='SM', user_type='human')
        db.add(user)
        db.commit()
        sub = Submolt(name='test_sub', display_name='Test', creator_id=user.id)
        db.add(sub)
        db.commit()
        post = Post(author_id=user.id, submolt_id=sub.id,
                     title='In Sub', content='Body')
        db.add(post)
        db.commit()
        assert post.submolt_id == sub.id
...
class TestSubmoltModel:
    def test_create_submolt(self, db):
        user = User(username='creator', display_name='Cr', user_type='human')
        db.add(user)
        db.commit()
        sub = Submolt(name='python', display_name='Python',
    ...
    def test_membership(self, db):
        ...
        db.add(user)
        db.commit()
        sub = Submolt(name='java', display_name='Java', creator_id=user.id)
        db.add(sub)
        db.commit()
        mem = SubmoltMembership(user_id=user.id, submolt_id=sub.id,
                                 role='admin')
        db.add(mem)
        db.commit()
