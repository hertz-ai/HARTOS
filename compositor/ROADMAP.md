...
- Dead-husk-aware boot health-check helper: a **real** Flask `test_client` / curl fetch of
  `/shell/static/*` (NOT inline-render) asserting 200 + non-empty body — carry the f294f52
  lesson into CI.
...
This must exist and be VM-proven **before** Tier-1/Tier-2 ever become default — it is the
only thing standing between a half-finished compositor and a black screen.
...
- Wire the EXISTING screen-capture consumers (`remote_desktop/frame_capture.py`,
  ...
  `core.ai_sensing.allowed('screen')` and refuse when cut — close the pre-existing dead-flag gap.
...
- Phase-1 supervisor latching active — a Smithay regression silently lands on today's cage
  behavior.
...
## Phase 4 — Glass shell as layer-shell surface — the GTK3→GTK4 host-window port
...
framing. Decide the single/dual-WebView model honestly.
...
- Pick ONE z-order model in code: (1) single layer-shell surface, overlays/orb co-planar with
  desktop (native windows sit ABOVE the orb — stated honestly), OR (2) two WebViews (background
...
- integration: if model (2), an A2UI card is z-**ABOVE** a placed native window (proves the
  dual-surface promise); if model (1), test asserts co-planar and the honest limitation is
  documented.
...
## Phase 5 — Native app windows (xdg-shell + XWayland) + honest install→real-window path

**Goal:** make HART-comp host real native toplevels (Wine/Flatpak/PWA via xdg-shell +
XWayland), with launch-success keyed on a **REAL** `window.opened` map event — never on
...
- integration: launch a Flatpak/PWA toplevel, assert `window.opened` fires and a handle is
  ...
  success).
- integration: Android `SummonApp` returns honest "unsupported/inert" — no phantom handle.
...
- Android/Wine/macOS/crx limits carried as honest openRisks, not claimed solved.
...
immutable-audit** gate (built in Phase 2), NOT a no-op pipeline. This is the moat, and the most
dangerous new privilege, so it ships only after the audit/guardrail controls exist and the
...
- Compositor render-loop CPU explicitly accounted in `resource_governor` external-CPU
  ...
  the yield trigger) — mirror the governor self-throttle fix.
...
**Goal:** add Freedesktop portals for native apps AND make the screen kill-switch genuinely
...
- Real session lock: `ext-session-lock-v1` in HART-comp (input grab + blank app surfaces + PAM
  re-auth) for Tier-1/2; honestly document Tier-3 cage lock is UX-only and that autologin
...
- `nixosTest`: `ext-session-lock-v1` blanks app surfaces + requires PAM re-auth at Tier-1;
  documented honest gap at Tier-3.
...
- behavioural: meet-copilot overlay only advertised/fired when `agent_voice_bridge` is real AND
  A2UI push works; otherwise honest no-op.
...
- No capability advertised whose dependency (voice bridge, LiveKit egress) is a stub — gated or
  labeled openRisk.
