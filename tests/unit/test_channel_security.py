...
class TestPairingManager:
    ...
    def test_generate_then_verify_across_separate_instances(self, tmp_path):
        """Real production pattern: each API request builds its own
        PairingManager (see integrations/social/api_channels.py), so a
        code generated by one instance must be verifiable by a later,
        unrelated instance sharing only the storage_path."""
        storage_path = tmp_path / "pairing_data.json"

        generator = PairingManager(storage_path=str(storage_path))
        code = generator.generate_pairing_code(user_id=100, prompt_id=200)

        verifier = PairingManager(storage_path=str(storage_path))
        session = verifier.verify_pairing("whatsapp", "user123", code)
