...
class TestTTLCacheTouchOnRead:
    """Touch-on-read semantics — locks in the 2026-05-08 incident fix.

    Prior behavior: TTL was set on write only.  An active recipe pipeline
    that read user_tasks[user_prompt] continuously could still hit
    KeyError after 7200s (2h) because reads didn't extend TTL.
    Symptom: TTS without matching text bubble (fallback path triggered
    when the recipe couldn't reach its own state).

    New behavior: every successful read via [] / .get() / setdefault()
    extends the entry's TTL.  Abandoned entries (no reads) still age
    out, preserving memory-leak-prevention.
    """

    def test_getitem_extends_ttl_on_active_access(self):
        """Continuous reads must keep the entry alive past the original TTL."""
        from core.session_cache import TTLCache
        # Short TTL so we can simulate a long pipeline in a fast test
        c = TTLCache(ttl_seconds=2)
        c['active'] = 'state'
        # Backdate to 1.5s ago, then read — touch resets to now
        c._timestamps['active'] -= 1.5
        _ = c['active']  # touch
        # Backdate AGAIN by 1.5s — without touch this would now be -3s old
        # (past TTL=2s).  With touch, age is 1.5s after the touch — alive.
        c._timestamps['active'] -= 1.5
        # Should still be alive (last touch was 1.5s ago, TTL=2s)
        assert c['active'] == 'state', (
            "active access did not extend TTL — touch-on-read regressed"
        )

    def test_get_extends_ttl(self):
        """.get() routes through __getitem__ so it should also touch."""
        from core.session_cache import TTLCache
        c = TTLCache(ttl_seconds=2)
        c['k'] = 'v'
        c._timestamps['k'] -= 1.5
        _ = c.get('k')  # touch via __getitem__
        c._timestamps['k'] -= 1.5
        # Last touch was 1.5s ago, still within 2s TTL
        assert c.get('k') == 'v', (
            ".get() did not propagate touch-on-read"
        )

    def test_setdefault_extends_ttl_on_hit(self):
        """setdefault() cache-hit path must also touch."""
        from core.session_cache import TTLCache
        c = TTLCache(ttl_seconds=2)
        c['k'] = 'original'
        c._timestamps['k'] -= 1.5
        result = c.setdefault('k', 'fallback')  # touch on hit
        assert result == 'original'
        c._timestamps['k'] -= 1.5
        # Last touch (via setdefault) was 1.5s ago, still alive
        assert c['k'] == 'original', (
            "setdefault did not extend TTL on cache hit"
        )

    def test_unaccessed_keys_still_expire(self):
        """The memory-leak-prevention guarantee must hold: keys with NO
        reads must still age out at the original TTL."""
        from core.session_cache import TTLCache
        c = TTLCache(ttl_seconds=1)
        c['abandoned'] = 'state'
        # NO reads happen.  Backdate past TTL.
        c._timestamps['abandoned'] -= 5
        # Cleanup pass should remove it
        c._cleanup_expired(time.monotonic())
        assert 'abandoned' not in c._data, (
            "unaccessed key did not expire — memory-leak prevention regressed"
        )

    def test_contains_does_not_touch(self):
        """__contains__ (`in` operator) must remain side-effect-free
        for predictable Python semantics.  Only [] / .get() / setdefault
        extend TTL."""
        from core.session_cache import TTLCache
        c = TTLCache(ttl_seconds=2)
        c['k'] = 'v'
        ts_before = c._timestamps['k']
        time.sleep(0.05)  # advance monotonic clock noticeably
        _ = 'k' in c  # `in` operator
        ts_after = c._timestamps['k']
        # Timestamp must NOT have moved — `in` is read-only
        assert ts_before == ts_after, (
            "__contains__ touched the timestamp — should be side-effect-free"
        )
