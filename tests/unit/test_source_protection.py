...
# ── Source Guard (inspect blocking) Tests ────────────────────────

class TestSourceGuards:
    """Tests for install_source_guards() — blocks inspect.getsource() on hevolveai."""

    def setup_method(self):
        """Reset inspect functions to originals before each test."""
        import inspect as _inspect
        from security.source_protection import (
            _original_getsource, _original_getsourcelines,
            _original_getsourcefile, _original_findsource,
        )
        _inspect.getsource = _original_getsource
        _inspect.getsourcelines = _original_getsourcelines
        _inspect.getsourcefile = _original_getsourcefile
        _inspect.findsource = _original_findsource

    def teardown_method(self):
        """Restore originals after each test."""
        self.setup_method()

    def test_guards_installed(self):
        """install_source_guards patches inspect module."""
        import inspect as _inspect
        from security.source_protection import (
            install_source_guards, _original_getsource,
            _guarded_getsource,
        )
        assert _inspect.getsource is _original_getsource
        install_source_guards()
        assert _inspect.getsource is _guarded_getsource

    def test_blocks_hevolveai_module(self):
        """inspect.getsource raises for hevolveai-attributed objects."""
        import inspect as _inspect
        from security.source_protection import install_source_guards
        install_source_guards()

        mock_obj = MagicMock()
        mock_obj.__module__ = 'hevolveai.core.engine'
        mock_obj.__name__ = 'Engine'

        with pytest.raises(OSError, match='source code not available'):
            _inspect.getsource(mock_obj)

    def test_blocks_hevolveai_subpackage(self):
        """Blocks subpackages like hevolveai.hivemind.protocol."""
        import inspect as _inspect
        from security.source_protection import install_source_guards
        install_source_guards()

        mock_obj = MagicMock()
        mock_obj.__module__ = 'hevolveai.hivemind.protocol'

        with pytest.raises(OSError):
            _inspect.getsource(mock_obj)

    def test_blocks_exact_hevolveai_module(self):
        """Blocks the top-level hevolveai module itself."""
        import inspect as _inspect
        from security.source_protection import install_source_guards
        from types import ModuleType
        install_source_guards()

        fake_mod = ModuleType('hevolveai')
        with pytest.raises(OSError):
            _inspect.getsource(fake_mod)

    def test_allows_non_protected_modules(self):
        """Non-protected modules pass through to real inspect."""
        import inspect as _inspect
        from security.source_protection import install_source_guards
        install_source_guards()

        # inspect.getsource on a real stdlib function should work
        source = _inspect.getsource(install_source_guards)
        assert 'def install_source_guards' in source

    def test_getsourcefile_returns_none_for_protected(self):
        """getsourcefile returns None for protected packages."""
        import inspect as _inspect
        from security.source_protection import install_source_guards
        install_source_guards()

        mock_obj = MagicMock()
        mock_obj.__module__ = 'hevolveai.sensors'
        assert _inspect.getsourcefile(mock_obj) is None

    def test_getsourcelines_blocks_protected(self):
        """getsourcelines raises for protected packages."""
        import inspect as _inspect
        from security.source_protection import install_source_guards
        install_source_guards()

        mock_obj = MagicMock()
        mock_obj.__module__ = 'hevolveai'

        with pytest.raises(OSError):
            _inspect.getsourcelines(mock_obj)

    def test_findsource_blocks_protected(self):
        """findsource raises for protected packages."""
        import inspect as _inspect
        from security.source_protection import install_source_guards
        install_source_guards()

        mock_obj = MagicMock()
        mock_obj.__module__ = 'hevolveai.learning'

        with pytest.raises(OSError):
            _inspect.findsource(mock_obj)

    def test_idempotent_install(self):
        """Calling install_source_guards multiple times is safe."""
        import inspect as _inspect
        from security.source_protection import (
            install_source_guards, _guarded_getsource,
        )
        install_source_guards()
        install_source_guards()
        install_source_guards()
    ...
    def test_does_not_block_similar_names(self):
        """'malicious_hevolveai' doesn't match — must be exact package."""
        import inspect as _inspect
        from security.source_protection import install_source_guards
        install_source_guards()

        mock_obj = MagicMock()
        mock_obj.__module__ = 'malicious_hevolveai.backdoor'
        # Should NOT block — not a hevolveai subpackage
        # Will fall through to real inspect which will fail on a mock
        # but with a different error (not "source code not available")
        try:
            _inspect.getsource(mock_obj)
        except OSError as e:
            assert 'source code not available' not in str(e)
        except TypeError:
            pass  # Expected for mock objects from real inspect
