...
template = """You are Hevolve, a highly intelligent education AI, developed by HertzAI, designed to answer questions, provide revisions, and teach various topics to students.
Your response should be meaniful and should not excide more than 200 words and should be as fast as posssible.
Below is an instruction that describes a task, paired with an input that provides further context. Write a response that appropriately completes the request.Tu
...
prompt = CustomPromptTemplate(input_variables=["input", "history", "intermediate_steps"],
...
def answer(question: str, user_id: int, conv_id: int, first_req: bool = False, list_of_document: list = [], persist_directory: str = config.PERSIST_DIR) -> str:
    ...
    agent_executor = AgentExecutor.from_agent_and_tools(
        agent=agent,
        tools=tools,
        verbose=True, memory=VectorStoreRetrieverMemory(retriever=db.as_retriever(search_kwargs={"score_threshold": .5,
                                                                                                 "metadatas": metas, "collection_name": collection_name}))
    )
