...
let
  ...
  hartWriteInstallConfig = pkgs.writeShellApplication {
    ...
    text = ''
      ...
      # ── hardware-local.nix: MACHINE-PROBED facts, ONE writer ──────────────
      # WHY A SECOND FILE (the parallel-path fix, steward 2026-07-31 "why are
      # they not canonicalised?"). local.nix is the FRONT-END's file: the CLI
      # writes --hostname into it, Calamares writes hostname/user/locale/
      # keyboard/autologin into it, in two different languages. Facts the
      # INSTALLER probes off the machine do not belong in a file two
      # front-ends both rewrite — that is precisely how the dual-boot clock
      # fix landed in the CLI and missed the GUI (task #24).
      #
      # This generator is ALREADY the one thing both front-ends call
      # (Calamares shells out to hart-write-install-config), so probed facts
      # get exactly one writer by putting them here. NixOS's own module
      # imports do the composition — no new mechanism, and no file with two
      # authors.
      #
      # Rewritten on EVERY run, unlike local.nix: re-probing is correct when
      # the machine changed (a Windows install appearing beside HART is the
      # whole point), and nothing human-authored lives here to preserve.
      _win=""
      ...
      done
      {
        echo '{ ... }:'
        echo '{'
        echo '  # GENERATED by hart-write-install-config from probing THIS'
        echo '  # machine. Safe to re-run; edit local.nix instead.'
        if [ -n "$_win" ]; then
          echo '  # A Windows bootloader is present. Windows keeps the hardware'
          echo '  # clock in LOCAL time, so HART must read it the same way or NTP'
          echo '  # steps the wall clock by the timezone offset on first sync'
          echo '  # (task #24 — the real-hardware hang).'
          echo '  time.hardwareClockInLocalTime = true;'
        fi
        echo '}'
      } > "$mnt/etc/nixos/hardware-local.nix"
      [ -n "$_win" ] && \
        ...
        outputs = { self, hart, ... }: {
          nixosConfigurations.hart = hart.lib.mkInstalledSystem {
            ...
            hardwareModules = [
              ...
              ./boot.nix
              # Machine-probed facts, written by this generator on every run
              # (dual-boot clock today). Separate from local.nix so probed
              # facts have ONE writer while both front-ends keep owning the
              # user's choices — see the hardware-local.nix comment above.
              ./hardware-local.nix
              ./local.nix
            ];
          };
