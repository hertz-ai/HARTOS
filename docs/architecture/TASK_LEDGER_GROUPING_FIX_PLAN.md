...
| `agent_id` | Mirrors `prompt_id`'s domain (int for humans, UUID for autonomous). | Always stored as **string** in `SmartLedger.agent_id` (`create_ledger_from_actions` does `agent_id = str(prompt_id)` explicitly). | The "agent_id == prompt_id" convention is the load-bearing identity that the dashboard groups by; keeping them physically the same string avoids one more denormalised mapping to maintain. |
...
### Plan-vs-implementation divergence (intentional, documented for the reader)
