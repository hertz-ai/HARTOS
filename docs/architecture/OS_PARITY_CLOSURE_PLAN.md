...
|---|---|---|
| **compositor build (crate-403)** | **HIGHEST-PRIORITY BLOCKER — gates ALL VM verification.** The compositor Rust build fetches its ~245 crates and gets **HTTP 403** on `js-sys 0.3.93` → the `Compositor nix build` job fails → **nixosTests cascade-SKIP** → the entire VM parity suite (driver/BIOS/Hyper-V/latency/`#42`/`#12`) never executes. Root-caused via run `33298320160` @ `ca645a43`. **PRECISE CAUSE (live-confirmed this session, curl from a dev box):** `https://crates.io/api/v1/crates/js-sys/0.3.93/download` → **403**, but `https://static.crates.io/crates/js-sys/js-sys-0.3.93.crate` → **200** (S3, 103 KB). crates.io now 403s the legacy `/api/v1/.../download` endpoint for non-cargo fetchers; the build's fetch routes through it instead of the working `static.crates.io` CDN. `js-sys` is a benign target-agnostic transitive (vendored, never compiled on native — NOT removable). **FIX (small, targeted — NOT a 245-crate vendoring job): bump the `crane` flake input.** The CI job builds via **crane's `vendorCargoDeps`** (`hart-comp.nix:173`), and crane constructs the crate download URL. The pinned `crane` input (`469fd08d…`, `flake.nix:51`) predates crane's switch to `static.crates.io`, so it fetches via the deprecated `/api/v1/.../download` that now 403s. Fix = `nix flake lock --update-input crane` to a rev whose `vendorCargoDeps` uses `static.crates.io` (the `buildRustPackage` fallback path uses nixpkgs `importCargoLock`, so `nixpkgs-rust` `ac62194c` may need the same bump). Requires a Nix env to run `flake lock` + CI-validate — NOT doable from the Windows dev box, and pushing an unvalidated flake-input bump to shared `main` is unsafe (would risk breaking concurrent builds). |
