...
|---|---|---|
| **compositor build (crate-403)** | **HIGHEST-PRIORITY BLOCKER — gates ALL VM verification.** `rustPlatform.importCargoLock` fetching the compositor's ~245 crates gets **HTTP 403** on `js-sys 0.3.93` from crates.io → the `Compositor nix build` job fails → **nixosTests cascade-SKIP** → the entire VM parity suite (driver/BIOS/Hyper-V/latency/`#42`/`#12`) never executes. `403≠429` and it persisted across a re-run → likely a persistent CI-egress/CDN block, not a transient rate-limit. `js-sys` is a benign target-agnostic transitive (vendored, never compiled on native — NOT removable). Root-caused via run `33298320160` @ `ca645a43`. Fix (in the crane/`buildRustPackage` domain — `flake.nix` / `hart-comp.nix`): offline `cargoVendorDir` vendoring (robust, "never fetch at build"), or a crates.io mirror/source-replacement, or fetch-retry (weaker for a hard 403). |
