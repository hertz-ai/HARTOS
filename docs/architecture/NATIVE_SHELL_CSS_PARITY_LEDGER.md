...
| CSS surface | effects | states | animations | native SceneNode mapping |
|---|---|---|---|---|
| `body.gpu-hardware #hart-voice-orb` | **TRANSFORM-ONLY by design**, so the orb's drop-shadow bloom AND the listening filter state (owned by `hartHero.js` / `voiceOrbViz.js`) are UNTOUCHED and can compose independently. This is a calm CSS float layered ON TOP of the canvas visualiser. Orb sits at `z-index:1450` (above `.hart-home` z30). Software floor: no CSS float/breathe transform. **NOT a gap, and NOT a dead orb (steward-corrected 2026-07-20):** the breathing pillar itself is carried by the CANVAS visualiser (`voiceOrbViz.js` render loop -- `breathe1`/`breathe2` modulate the halo + core, driven by an UNGATED requestAnimationFrame with no gpu-hardware/webkit-flat gate), so the orb breathes on the software floor exactly as intended -- the "cheap heartbeat" the calm-but-alive rule prescribes. This CSS layer is only the ADDITIONAL float/scale on capable hardware. **NATIVE CONSEQUENCE:** the native orb must carry breathing in the ORB ITSELF (shader core/halo modulation, always on, honouring the `hart_orb_breathing` pref + reduced-motion), never gated on a GPU tier flag. | gpu-hardware; `--hart-motion-orb:paused`; reduced-motion (`animation:none !important`) | `vFloat calc(9s * speed) ease-in-out infinite, vBreathe calc(4s * speed) ease-in-out infinite`; play-state `var(--hart-motion-orb)` | **Orb** transform layer -- M3 |
