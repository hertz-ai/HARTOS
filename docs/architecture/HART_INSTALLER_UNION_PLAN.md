...
| 5 | **Rebranded Calamares** as the graphical path: HART branding descriptor (product name, logo, slideshow, QSS) + a HART extension of `calamares-nixos-extensions` whose config module writes the union config. GUI and CLI call the SAME generator (`hart-write-install-config --mounted` seam exists for exactly this) — one writer for the installed configuration | ⬜ not started; the generator seam is ready |
