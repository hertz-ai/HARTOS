...
| 25 | Package every common OS feature | `native-subsystems.nix`, `power-actions.nix`, `storage-filesystems.nix`, `network-wifi.nix`, `notify.nix`, `portal-screencast.nix`, `driver-matrix.nix` (`/api/shell/kernel` cross-checked against the guest's own `/proc/modules`; `/api/shell/gpu` must answer present=False on a GPU-less VM rather than 503 or a fabricated 0) | EXISTS — audit method corrected 2026-08-04: the original gap list probed route PREFIXES, so it reported swap/zram and thermal as missing when both already ship (`/api/shell/memory`, `/api/shell/system/metrics`). Re-audited across all 241 `/api/shell/*` route BODIES. GPU and kernel/modules now closed; 3 real gaps remain (file sharing, containers/VM, sshd) |
