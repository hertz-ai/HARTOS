...
existing logic in Nunba-Companion — no parallel path), **zero regression** (run the relevant
...
- **Transport (server):** `agent_ui_update` :912 (allowlist already accepts custom types :932);
  `compose_home(mood=)` :1220/1246; `/api/home/compose` :6432 (drops mood — one-line fix);
...
if(mp) HartPalette.paint(mp)` — no-op on miss (graceful). (c) `/api/home/compose` (:6438) pass
...
`spec.props` as label/value rows — reuse `_esc`+`dsBtn`+form-iter; mirror Nunba AgentOverlay
