...
| The agent-engine blueprint is absent from the live backend | `list_routes` on the node: **837 routes registered, 0 of them agent-engine**, and `/api/agent-engine/ledger/stats` 404s on every port tried. That endpoint is the SHADOW-IMMUNE probe `_tool_impls` uses to correct a known MCP false negative (its own docstring: MCP reported `daemon_enabled=false, _tick_count=0` while the ledger showed 5265 tasks progressing). With the blueprint unregistered the correction cannot run, so `agent_status`'s `daemon_enabled: false, daemon_tick_count: 0` is the UNRELIABLE view and the daemon's true state is not knowable from outside the process on this node | 2026-09-07, measured on hardware |
