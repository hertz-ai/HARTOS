...
| The shell's main thread has been in a busy loop since boot, and it is NOT the animation I first blamed | On the live node WebKit's main thread sat at **487 of 500 jiffies, 97.4% of a core, continuously from boot**, while `hart-comp` idled at **2.2%**. I named the ungated onboarding orb (`.hob-orb`, two large box-shadows scaling forever) as the cause. WRONG, and the measurement that settles it is context switches: over 10s at 99% CPU the thread showed **voluntary +37, nonvoluntary +173**, i.e. it yields **3 times a second** and is preempted 17. A rAF or CSS-animation loop yields ~60/s. This is a SYNCHRONOUS BUSY LOOP that almost never yields, and the idle compositor agrees from the other end: an animation at 60fps would have produced frames to composite, and there were none. Ruled out by measurement, not reading: the JS orb (stand-down live and served), the bloom (composes once to canvas), the SSE drain (self-terminating; live stream carried 0 data lines in 15s), lottie (0 loadAnimation), the lock screen (not `.active`), CSS animations generally (`webkit-flat` on the body, sw-paint gate emitted), and JS exceptions (no console errors in the journal). The orb gate fix still lands, on the gate's own criterion, but it does not address this. CAUSE STILL UNKNOWN: no profiler on the node (perf/gdb/eu-stack all absent) and every in-place experiment needs a page reload, which needs the graphical session to restart | 2026-09-10, measured on hardware |
