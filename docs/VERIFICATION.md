...
| Daemon goals produce work, and the work never reaches an executor | traced end to end on the box 2026-09-07. WORKS: `hive_benchmark_prover` really created 6 tasks ("Benchmark shard 0 of ensemble_mmlu", priority 70) and persisted them to `agent_data/hive_tasks.json`. BROKEN: all 6 sit `pending`, none ever dispatched, and the governor has logged no dispatch this boot. The ONLY tick is `resource_governor._proactive_check_tasks`, which opens `if self._mode != MODE_IDLE: return`. An executor IS available and waiting (`chs_85d7322feaa9`, status `idle`). So the queue is full, the worker is idle, and nothing connects them. This is also a SECOND, independent reason the copilot is idle: it polls `/api/hive/session/tasks` for `pending`, and that list stays empty because delivery never happens, quite apart from the missing credential | 2026-09-07, measured on hardware |
