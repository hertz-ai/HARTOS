...
| Daemon goals produce work, and the work never reaches an executor | traced end to end on the box 2026-09-07. WORKS: `hive_benchmark_prover` really created 6 tasks and persisted them; `match_session` WOULD match one (proved locally against the live session's exact shape: benchmark tasks carry no `files_scope`, so the language filter passes and an idle session scores 2.0+); `get_session` WOULD resolve it (singleton fallback). BROKEN: all 6 sit `pending`, none dispatched, no dispatch logged this boot. The only tick is `resource_governor._proactive_check_tasks`, gated `if self._mode != MODE_IDLE: return`, and the node is not idle by any measure: 70.8% busy on 8 cores, load 5.10, llama-server at ~2.5 cores and WebKitWebProcess at ~1. Compounding it, `_get_idle_ms_linux()` only tries `xprintidle`, an X11 tool absent from this Wayland-only OS, so OS idle detection returns None and the governor falls back to a timestamp only a genuine user turn bumps. NOT established: the live mode value, which needs the singleton inside the backend process and has no route | 2026-09-07, measured on hardware |
