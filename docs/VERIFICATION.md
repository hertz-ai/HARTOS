...
| Daemon goals produce work, and the work never reaches an executor | traced on the box 2026-09-07, and the CAUSE IS STILL OPEN. WORKS: `hive_benchmark_prover` created 6 tasks and persisted them; `match_session` would match one (proved locally against the live session's exact shape, returning `chs_85d7322feaa9`); `get_session` would resolve that id via its singleton fallback. BROKEN: all 6 sit `pending`, none dispatched, no dispatch logged this boot. RULED OUT rather than assumed: I first blamed the `MODE_IDLE` gate, and the evidence does not support it. `user_idle` should be true (only `/chat` bumps the activity timestamp via `mark_view`, and no chat turn has happened), `ext_cpu` is roughly 44% of 8 cores against a 0.85 threshold, and memory is 30.3% against 0.90 -- so `_target_mode_for` should return IDLE. What remains untested is whether the live `get_available_sessions()` actually returns the local session, and the live mode value. Both need the singletons inside the backend process and there is no route to either. Recorded as OPEN rather than guessed a third time | 2026-09-07, break confirmed on hardware; cause not yet isolated |
