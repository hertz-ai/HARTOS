...
| An OS app installs from the store and LAUNCHES | the real `AppInstaller._install_nix`, unmocked, on the Samsung box: package absent -> installed -> `install_path` a real store path that exists on disk -> present in `nix-env -q` -> the binary runs (`Hello, world!`). The shipped invocation could never have worked: `nix-env -iA nixpkgs.<pkg>` resolves through a CHANNEL and a flake-built node has none, so every nix install failed with `attribute 'nixpkgs' ... not found`. All 176 installer tests fake the package manager, so none of them saw it | 2026-09-07, measured on hardware |
...
| 19 | Input-to-photon meets the per-surface budgets | `latency.rs` prints `hart-latency component=... verdict=PASS/FAIL` per 10s window, but ONLY for input that causes a repaint. Pointer motion alone is measured to cost zero frames (above), so this needs input over an INTERACTIVE target: hover the orb (hover lift), click a home card, or scroll a row. A human at the machine settles it in a minute; synthetic motion cannot | node with a GPU + a hand |
