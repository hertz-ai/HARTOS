#!/usr/bin/env python3
"""
HART OS PXE Boot Server - TFTP + HTTP for network booting.

Serves the HART OS kernel, initrd, and autoinstall configs so target
machines can boot and install HART OS over the network.

Usage:
    sudo python hart-pxe-server.py --iso /path/to/hart-os.iso [--port 8888] [--interface eth0]
...
Flow:
    ...
    5. Ubuntu installer runs with HART OS autoinstall
...
logger = logging.getLogger('hart-pxe')
...
def extract_iso(iso_path: str, output_dir: str):
    ...
    mount_point = tempfile.mkdtemp(prefix='hart-iso-')
...
def _generate_self_signed_cert(cert_path: str, key_path: str, hostname: str = None):
    ...
    subject = f"/CN={hostname}/O=HART OS PXE Server"
    ...
    try:
        ...
        try:
            ...
            name = x509.Name([
                ...
                x509.NameAttribute(NameOID.ORGANIZATION_NAME, "HART OS PXE Server"),
            ])
...
def print_dnsmasq_hint(server_ip: str, serve_dir: str):
    ...
    logger.info(
        ...
        "  │   apt install dnsmasq                                  │\n"
        "  │   # /etc/dnsmasq.d/hart-pxe.conf:                     │\n"
        "  │   port=0                                               │\n"
        "  │   dhcp-range=<subnet>,proxy                            │\n"
        "  │   pxe-service=x86PC,\"HART OS\",pxelinux                 │\n"
        "  │   enable-tftp                                          │\n"
        "  │   tftp-root=%s                             │\n"
        "  └─────────────────────────────────────────────────────────┘\n",
...
def main():
    parser = argparse.ArgumentParser(
        description='HART OS PXE Boot Server',
    )
    parser.add_argument('--iso', help='Path to HART OS ISO file')
    ...
    logger.info("Starting HART OS PXE server...")
