...
func setupPXEConfig(serveDir, serverIP string, httpPort int) {
	...
	config := fmt.Sprintf(`DEFAULT hyve-auto
PROMPT 1
TIMEOUT 100
...
func setupAutoinstallDir(serveDir string) {
	...
	// In the repo layout: deploy/distro/pxe/hyve-pxe-server-go/ -> deploy/distro/autoinstall/
...
func findRepoAutoinstallDir() string {
	...
	if err == nil {
		...
		// deploy/distro/pxe/hyve-pxe-server-go/ -> deploy/distro/autoinstall/
	...
	if runtime.GOOS == "linux" {
		candidate := "/opt/hyve/deploy/distro/autoinstall"
		if dirExists(candidate) {
			return candidate
		}
	}
