#!/usr/bin/env bash
# ============================================================
# HART OS Daily Backup
#
# Backs up critical node data to /var/backups/hart/:
...
#
# Triggered by hart-backup.timer (systemd, daily 03:00).
# ============================================================

set -euo pipefail

DATA_DIR="/var/lib/hart"
BACKUP_DIR="/var/backups/hart"
LOG_DIR="/var/log/hart"
LOG="$LOG_DIR/backup.log"
RETENTION_DAYS=7

TIMESTAMP=$(date +"%Y%m%d-%H%M%S")
BACKUP_NAME="hart-backup-${TIMESTAMP}"
...
done < <(find "$BACKUP_DIR" -name "hart-backup-*.tar.gz" -mtime +"$RETENTION_DAYS" -type f 2>/dev/null)
...
# ─── Summary ───
TOTAL_BACKUPS=$(find "$BACKUP_DIR" -name "hart-backup-*.tar.gz" -type f 2>/dev/null | wc -l)
