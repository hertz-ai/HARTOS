#!/usr/bin/env python3
"""
HyveOS OTA Update Service - Automated update mechanism.
...
Runs as hyve-update.service triggered by hyve-update.timer (daily).
"""
...
logger = logging.getLogger('hyve-update')
...
DEFAULT_UPDATE_URL = 'https://api.github.com/repos/hevolve-ai/hyve/releases/latest'
UPDATE_URL = os.environ.get('HYVE_UPDATE_URL', '')
if not UPDATE_URL:
    try:
        with open(os.path.join(CONFIG_DIR, 'hyve.env')) as _f:
            for _line in _f:
                if _line.startswith('HYVE_UPDATE_URL='):
...
class HyveUpdateService:
    """OTA-style update mechanism for HyveOS."""
    ...
    def check_for_updates(self) -> dict:
        ...
        try:
            ...
            req.add_header('User-Agent', 'HyveOS-Updater/1.0')
            ...
            for asset in assets:
                name = asset.get('name', '')
                if name.endswith('.tar.gz') and 'hyve-os' in name:
    ...
    def download_update(self, download_url: str, checksum_url: str = None) -> str:
        ...
        """
        temp_dir = tempfile.mkdtemp(prefix='hyve-update-')
    ...
    def apply_update(self, bundle_path: str, version: str = None) -> bool:
        ...
        try:
            ...
            subprocess.run(['systemctl', 'stop', 'hyve.target'], check=False)

            # Extract update
            logger.info("Extracting update...")
            with tarfile.open(bundle_path, 'r:gz') as tar:
                # Extract to temp dir first
                temp_extract = tempfile.mkdtemp(prefix='hyve-extract-')
            ...
            subprocess.run(['systemctl', 'start', 'hyve.target'], check=True)
    ...
    def rollback(self, backup_dir: str) -> bool:
        ...
        try:
            ...
            subprocess.run(['systemctl', 'start', 'hyve.target'], check=False)
