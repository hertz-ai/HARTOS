#!/usr/bin/env bash
# ============================================================
# HART OS Tamper-Evident Boot Audit Log
#
...
#
# Usage: hart-boot-audit.sh <NODE_ID> <TIER>
# Called by hart-first-boot.sh after all services are up.
# ============================================================
...
LOG="/var/log/hart/boot-audit.log"
mkdir -p "$(dirname "$LOG")"
exec > >(tee -a "$LOG") 2>&1
...
SERVICES=""
for svc in hart-backend hart-discovery hart-agent-daemon hart-vision hart-llm; do
...
# HART OS first-boot keys are raw 32-byte files
