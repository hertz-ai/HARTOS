#!/usr/bin/env python3
"""
HyveOS System Tray - Desktop indicator showing node status.
...
def view_logs(_icon=None, _item=None):
    ...
    terminal_cmds = [
        ["gnome-terminal", "--", "journalctl", "-u", "hyve-*", "-f", "-n", "100"],
        ["xterm", "-e", "journalctl -u 'hyve-*' -f -n 100"],
        ["konsole", "-e", "journalctl", "-u", "hyve-*", "-f", "-n", "100"],
    ]
...
def restart_services(_icon=None, _item=None):
    ...
    subprocess.run(
        ["pkexec", "systemctl", "restart", "hyve.target"],
        capture_output=True,
    )
...
def stop_services(_icon=None, _item=None):
    ...
    subprocess.run(
        ["pkexec", "systemctl", "stop", "hyve.target"],
        capture_output=True,
    )
...
class HyveTray:
    """System tray indicator for HyveOS."""
    ...
    def _get_node_id(self):
        ...
        key_path = "/var/lib/hyve/node_public.key"
        try:
        ...
        while self._running:
            ...
            if new_status != self.status:
                ...
                if self.icon:
                    ...
                    self.icon.title = f"HyveOS - {status_labels.get(self.status, self.status)}"
