...
func TestDirectoryConstants(t *testing.T) {
	...
	if configDir != "/etc/hart" {
		t.Errorf("configDir: expected /etc/hart, got %s", configDir)
	}
	if dataDir != "/var/lib/hart" {
		t.Errorf("dataDir: expected /var/lib/hart, got %s", dataDir)
	}
	if installDir != "/opt/hart" {
		t.Errorf("installDir: expected /opt/hart, got %s", installDir)
	}
}
