...
const (
	...
	envFilePath        = "/etc/hyve/hyve.env"
	httpGetTimeout     = 5 * time.Second
	httpPostTimeout    = 30 * time.Second
)

// getBackendPort reads HYVE_BACKEND_PORT from /etc/hyve/hyve.env.
...
func getBackendPort() int {
	...
	for scanner.Scan() {
		...
		if strings.HasPrefix(line, "HYVE_BACKEND_PORT=") {
...
// getBackendPortFromReader parses HYVE_BACKEND_PORT from an arbitrary reader.
