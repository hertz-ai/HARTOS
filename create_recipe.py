...
def visual_based_execution(task_description: str, user_id: int, prompt_id: int):
    ...
    try:
        ...
        # Check for message_2_user and send to user if needed
        if 'message_2_user' in last_message['content'].lower():
            try:
                json_obj = retrieve_json(last_message['content'])
                if json_obj and 'message_2_user' in json_obj:
...
def call_visual_task(task_description: str, user_id: int, prompt_id: int, data: list = None, date: datetime = None):
    ...
    # Check conditions for sending to external visual agent
    if data and any(obj.get("zeroshot_label") == 'Video Reasoning' for obj in data) and (now - date) > timedelta(seconds=30):
        data_to_send = json.dumps({
            'task_description': task_description,
            'user_id': user_id,
        ...
        # Use internal visual processing
        current_app.logger.info("Using internal visual processing")
        return visual_based_execution(task_description, user_id, prompt_id)
...
def create_agents(user_id: str,task,prompt_id) -> Tuple[autogen.ConversableAgent, autogen.ConversableAgent]:
    ...
    def get_user_camera_inp_by_mins(minutes: Annotated[
        ...
        visual_context = get_visual_context(user_id, minutes)
        current_app.logger.info(f'GOT RESPONSE AS {visual_context}')
        if not visual_context:
            visual_context = 'User\'s camera is not on. no visual data'
        return visual_context

    helper.register_for_llm(name="get_user_camera_inp_by_mins", description="Retrieve the user's visual camera input from the past specified minutes.")(get_user_camera_inp_by_mins)
    assistant.register_for_execution(name="get_user_camera_inp_by_mins")(get_user_camera_inp_by_mins)
    ...
    def state_transition(last_speaker, groupchat):
        ...
        try:
            visual_context = get_visual_context(user_id, 5)
            
            if visual_context:
                current_app.logger.info('Visual context available for conversation')
        except Exception as e:
            current_app.logger.error(f'Error getting visual context in state transition: {e}')
