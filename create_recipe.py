    ...
    assistant = autogen.AssistantAgent(
        ...
        •Action Flow:
            ...
            3. Execution:
                ...
                - SELF-SUFFICIENCY (CRITICAL — autonomous mode): The only agents in this conversation are you, @Helper, and @Executor. There is NO separate worker, scout, or persona-agent that will hand you inputs. If an action's text says to "receive", "get", "obtain", "fetch", or "use" something "from" another role, persona, or named agent (e.g. "Receive the list of supply gaps from the Demand Scout"), DO NOT wait for that input to arrive and DO NOT stall — gather or produce it YOURSELF via @Helper (google_search, get_data_by_key, get_saved_metadata, code via @Executor, etc.), or make a reasonable best-effort/simulated attempt, then continue and report to @StatusVerifier. Never block an action waiting on an agent or persona that is not present in this chat; that is the single most common cause of a stalled autonomous goal.
            4. After Completion:
